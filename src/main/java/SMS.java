...
public class SMS {
	...
	String SMSC_LOGIN    = "programmemann";     // логин клиента
	String SMSC_PASSWORD = "programmemannpassword";  // пароль или MD5-хеш пароля в нижнем регистре
	...
	public SMS(String login, String password) {
		SMSC_LOGIN    = login;
		SMSC_PASSWORD = password;
	}

	public SMS(String login, String password, String charset) {
		SMSC_LOGIN    = login;
		SMSC_PASSWORD = password;
		SMSC_CHARSET  = charset;
	}

	public SMS(String login, String password, String charset, boolean debug) {
		SMSC_LOGIN    = login;
		SMSC_PASSWORD = password;
		SMSC_CHARSET  = charset;
		SMSC_DEBUG    = debug;
		...
		try {
			...
			if (SMSC_DEBUG) {
				if (m[1] != "" && Integer.parseInt(m[1]) >= 0) {
					Timestamp currentTimestamp = new Timestamp(Integer.parseInt(m[1]));
